...
def run_source(source: str, scrape: bool, load: bool, scrape_limit: int | None, with_travel_times: bool) -> None:
    cmd = [PY, "pipeline.py"]
    if scrape:
        cmd.append("--scrape")
    if load:
    ...
    if with_travel_times and source == "osm":
        cmd.append("--with-travel-times")
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help="inclut le calcul des temps de trajet OSM (OSRM) -- cf. scripts/osm/README.md",
    )
    ...
    for source in order:
        if source in sources:
            run_source(source, args.scrape, args.load, args.scrape_limit, args.with_travel_times)
