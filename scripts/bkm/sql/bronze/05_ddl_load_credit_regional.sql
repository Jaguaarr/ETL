...
===============================================================================
Colonnes CSV reelles (ordre alphabetique impose par build_csv_headers()) :
  report_title, report_url, pdf_filename, page_number, row_number,
  Code rayon d'action, Credit %, Credit Montant, Depots %, Depots Montant,
  Nombre Guichets, Rayon d'action

Incremental (upsert sur code_rayon_action + periode) : un PDF publie par
mois, on accumule l'historique. periode (MM-AAAA) est extraite de
report_title au chargement (regex sur "MM-AAAA").
===============================================================================
*/
...
CREATE TABLE IF NOT EXISTS bronze.bkam_credit_regional (
    ...
    periode                   text,   -- MM-AAAA, extraite de report_title
...
-- La table de staging respecte EXACTEMENT l'ordre des colonnes du CSV
...
SELECT
    report_title, report_url, pdf_filename, page_number, row_number_src,
    btrim(code_rayon_action), btrim(rayon_action), nombre_guichets,
...
WHERE code_rayon_action IS NOT NULL AND btrim(code_rayon_action) <> ''
