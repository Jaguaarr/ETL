/*
===============================================================================
Silver Layer - Transform - Google Maps mobilite
===============================================================================
*/
...
BEGIN
    IF to_regclass('monitoring.etl_log') IS NOT NULL THEN
        PERFORM monitoring.log_etl_start('gglmaps', 'silver', 'gglmaps_mobility', gen_random_uuid());
    END IF;
END $$;

TRUNCATE TABLE silver.gglmaps_mobility_rejects;
INSERT INTO silver.gglmaps_mobility_rejects (name, lat, lon, reject_reason)
SELECT name, lat, lon,
    CASE
...
BEGIN
    SELECT count(*) INTO v_rows FROM silver.gglmaps_mobility;
    IF to_regclass('monitoring.etl_log') IS NOT NULL THEN
        PERFORM monitoring.log_etl_end('gglmaps', 'silver', 'gglmaps_mobility', v_rows, 'SUCCESS', NULL);
    END IF;
    RAISE NOTICE 'silver.gglmaps_mobility : % lignes', v_rows;
END $$;
