...
----------------------------
Orchestre le pipeline complet Google Maps : scraping (Playwright direct,
POIs + mobilite) -> bronze -> silver -> gold. Necessite
scripts/hcp/scraping/scrape_geo_reference.py deja execute (centroides
communes = points de recherche). Aucune cle API requise -- cf.
scripts/gglmaps/README.md pour la methode reellement utilisee (pas l'API
Places) et l'avertissement CGU associe.
...
def scrape(limit: int | None) -> None:
    for script in ("scrape_places.py", "scrape_places_mobility.py"):
        cmd = [PY, script, "--all", "--resume"]
        if limit:
            cmd += ["--limit", str(limit)]
...
def load_sql() -> None:
    sql_files = [
        ...
        GGLMAPS_DIR / "sql" / "bronze" / "03_ddl_bronze_mobility.sql",
        GGLMAPS_DIR / "sql" / "bronze" / "04_load_bronze_mobility.sql",
        GGLMAPS_DIR / "sql" / "silver" / "01_ddl_silver.sql",
        GGLMAPS_DIR / "sql" / "silver" / "02_transform_silver.sql",
        GGLMAPS_DIR / "sql" / "silver" / "03_ddl_silver_mobility.sql",
        GGLMAPS_DIR / "sql" / "silver" / "04_transform_silver_mobility.sql",
        GGLMAPS_DIR / "sql" / "gold" / "01_ddl_gold.sql",
        GGLMAPS_DIR / "sql" / "gold" / "02_transform_gold.sql",
        GGLMAPS_DIR / "monitoring" / "01_quality_checks.sql",
    ]
    for sql_file in sql_files:
        run(["psql", "-v", "ON_ERROR_STOP=1", "-f", str(sql_file.relative_to(ROOT))], cwd=ROOT)
    run(["psql", "-c", "SELECT monitoring.run_quality_checks_gglmaps();"], cwd=ROOT)
